...
function Lista({ itens, vazio, pessoaDe, descrever }) {
  if (itens.length === 0) {
    return <p className="py-10 text-center text-sm text-[#5f5e5e]">{vazio}</p>;
  }
  return (
    <ul className="divide-y divide-[#f0e6e6]">
      {itens.map((item) => {
        const pessoa = pessoaDe(item);
        return (
          <li key={item.id} className="py-4 flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm text-[#1a1c1c]">
                ...
                <span className="text-[#8f6f6e]">#{pessoa.codigo}</span>
              </p>
...
function ModalRegistroConsultas({
  feitas,
  recebidas,
  abaInicial = "feitas",
  onClose,
  ...
  return (
    <Modal
      ...
      onClose={onClose}
    >
      <div className="flex p-1 bg-white border border-[#e2d6d6] rounded-xl gap-1 mb-2">
        {abas.map((op) => (
          <button
            key={op.val}
            type="button"
      ...
      {aba === "feitas" ? (
        ...
        <Lista
          ...
          pessoaDe={(item) => item.quemViu}
          descrever={(item) =>
            item.permissao === "veterinario"
              ? `${item.quemViu.papel} que viu seu contato no atendimento.`
